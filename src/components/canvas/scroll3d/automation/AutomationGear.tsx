...
export const GEAR_MODEL = '/models/gear/automation.glb';

const AutomationGear: React.FC<AutomationRefs> = ({ progress, pose, kf }) => {
  const modelRef = useRef<Group>(null);
  const { scene } = useGLTF(GEAR_MODEL);
  const current = useRef({ x: 40, y: 4, z: 0, s: 7 });
