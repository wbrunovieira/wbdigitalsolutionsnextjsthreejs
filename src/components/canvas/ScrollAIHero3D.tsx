 ...
 *
 * HERO (scroll ~0): unchanged behaviour — the AI model floats/spins, the
 *   dodecahedron "balls" swarm toward the mouse with a moving rect-area light.
...
// Model choreography in world space (camera at z = 100). PLACEHOLDERS — tune.
...
type ModelPose = { x: number; y: number; z: number; s: number };
...
const AnimatedInstancedMesh: React.FC<BallsProps> = ({ lightRef, progress, target, modelPose }) => {
...
const FloatingModel: React.FC<SharedRefs> = ({ progress, modelPose, kf }) => {
  ...
  useFrame((state, delta) => {
    ...
    modelPose.current = { x: pr.x, y: pr.y, z: pr.z, s: pr.s };
  });
