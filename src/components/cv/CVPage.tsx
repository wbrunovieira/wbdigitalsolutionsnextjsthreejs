...
const CVHero3D = dynamic(() => import("./CVHero3D"), { ssr: false });
...
const CVPage: React.FC = () => {
  ...
  const swap = reduce
    ...
    : { initial: { opacity: 0, y: 18 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -18 }, transition: { duration: 0.4, ease: [0.22, 1, 0.36, 1] as const } };

  const hero = t.hero[audience];

  return (
    <main className="relative min-h-screen overflow-x-hidden bg-modern-gradient text-white">
      {/* Depth layers (on-brand, like the home hero) */}
      ...
      <div className="relative z-10">
        ...
        <header className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-6">
          <a href="#top" className="group flex items-center gap-3">
            ...
            <span className="flex flex-col leading-tight">
              ...
              <span className="text-[11px] uppercase tracking-[0.2em] text-white/50">CV · {t.headline.split(" · ")[0]}</span>
            </span>
          </a>
        ...
        {/* ===== Hero ===== */}
        <section id="top" className="relative mx-auto grid max-w-6xl items-center gap-8 px-6 pb-10 pt-6 lg:min-h-[74vh] lg:grid-cols-[1.15fr_0.85fr]">
          <div className="relative z-10">
            {/* Availability badge */}
            ...
            {/* Name — the anchor of the page, always in evidence */}
            ...
            <div className="mb-8 mt-8">
              <span className="mb-2 block text-sm font-semibold text-white/85">{t.toggle.intro}</span>
              <div className="inline-flex rounded-full border border-yellowcustom/40 bg-white/5 p-1 ring-1 ring-yellowcustom/15 backdrop-blur-sm">
                {(["engineering", "sales"] as Audience[]).map((a) => {
                  const active = audience === a;
                  const Icon = a === "engineering" ? Code2 : LineChart;
                  return (
                    <button
                      key={a}
                      onClick={() => setAudience(a)}
                      aria-pressed={active}
                      className={`relative flex items-center gap-1.5 rounded-full px-5 py-2 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellowcustom/70 ${
                    ...
                    >
                      ...
                      <Icon aria-hidden="true" className={`relative z-10 h-4 w-4 transition-colors ${active ? "text-primary" : "text-yellowcustom/80"}`} />
                      <span className={`relative z-10 transition-colors ${active ? "text-primary" : "text-white/85"}`}>
                        {a === "engineering" ? t.toggle.engineering : t.toggle.sales}
                      </span>
                    </button>
                  );
                })}
              ...
              <p className="mt-2 flex items-center gap-1.5 text-xs text-white/50">
                <MousePointerClick aria-hidden="true" className="h-3.5 w-3.5 text-yellowcustom/70" />
                {t.toggle.hint}
              </p>
            </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={audience} {...swap}>
                <h2 className="font-black leading-[0.95] tracking-tight" style={{ fontSize: "clamp(1.9rem, 4.8vw, 3.4rem)" }}>
                  {hero.title}
                </h2>
                <p className="mt-4 flex items-center gap-2 text-xl font-semibold text-yellowcustom">
          ...
          <div className="relative h-[280px] lg:h-[440px]">
            <div className="pointer-events-none absolute inset-0 -z-10 rounded-full bg-custom-purple/20 blur-3xl" aria-hidden="true" />
            <CVHero3D />
          </div>
        </section>
              ...
              {t.stats[audience].map((s) => {
                ...
                return (
                  <div key={s.id} className="group relative flex flex-col gap-2 bg-primary/20 p-6 backdrop-blur-sm transition-colors hover:bg-custom-purple/15">
                    ...
                    <span className="text-xs leading-snug text-white/55">{s.label}</span>
                  </div>
                );
              })}
            </motion.div>
        ...
        <Section index={1} title={t.sections.summary} icon={Sparkles}>
          <AnimatePresence mode="wait">
            <motion.p key={audience} {...swap} className="max-w-3xl text-lg leading-relaxed text-white/80">
              {t.summary[audience]}
            </motion.p>
          </AnimatePresence>
        </Section>

        {/* ===== Audience-led section: Stack (eng) | Commercial (sales) ===== */}
        <AnimatePresence mode="wait">
          {audience === "engineering" ? (
            <motion.div key="stack" {...swap}>
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-4 flex items-start justify-between">
                    ...
                    <ArrowUpRight aria-hidden="true" className="h-5 w-5 text-white/30 transition-all duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-yellowcustom" />
                  </div>
              ...
              return (
                <motion.div key={exp.company + exp.period} {...reveal(i * 0.06)} className="relative mb-10 last:mb-0">
                  ...
                  <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                    ...
                    <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-xs text-white/55">{exp.period}</span>
                  </div>
                  <p className="mt-1 text-sm font-semibold text-yellowcustom">
                    {exp.company} <span className="font-normal text-white/40">· {exp.location}</span>
                  </p>
                  {(() => {
                    const bullets = exp.bullets[audience];
                    ...
                    // No bullets for the active audience — condense to the one-line summary.
                    return <p className="mt-3 text-sm leading-relaxed text-white/60">{exp.summary}</p>;
                  })()}
                  {exp.tags && (
        ...
        {/* ===== Story ===== */}
        <Section index={5} title={t.sections.story} icon={Sparkles}>
          <div className="relative max-w-3xl space-y-4 rounded-3xl border border-white/10 bg-white/[0.02] p-8">
            <span className="pointer-events-none absolute left-5 top-3 select-none font-mono text-7xl leading-none text-yellowcustom/20" aria-hidden="true">
              &ldquo;
            </span>
            {t.story.map((p, i) => (
              <motion.p key={i} {...reveal(i * 0.08)} className="relative text-base leading-relaxed text-white/75">
                {p}
              </motion.p>
            ))}
          </div>
        ...
        {/* ===== Education + Languages ===== */}
        <Section index={6} title={t.sections.education} icon={GraduationCap}>
              ...
              <ul className="space-y-3">
                {t.education.map((e) => (
                  ...
                  >
                    ...
                    <span className="shrink-0 text-xs text-white/40">{e.year}</span>
                  </li>
                ))}
              </ul>
            </div>
